...
import { ClassAttributes, InputHTMLAttributes, ReactNode, DetailedReactHTMLElement } from 'react';
...
let bundleToDebug: { [key: string]: any } | undefined = undefined;
if (debugUseLocalServerBundle) {
    bundleToDebug = require('./server-bundle');
}
...
type ApplicationContextType = {
    componentDidRenderServerSideFuncs: any[],
    tasks: any[],
    firstRun: boolean,
    cssUrls: string[],
    jsUrls: string[],
    ...
    relativeUrl: string;
    isAmp: boolean;
    addComponentDidRenderServerSideFunc: (func: any) => void;
    addTask: (promise: Promise<any>) => void;
};

type ComponentContextMetaData = {
    strategy: 'asyncRedux' | 'default',
    timeout: number;
    baseUrl: string;
    applicationContextServer: {
        relativeUrl: string;
...
type ComponentContext = {
    ...
    data: Object;
    props: Object;
    metadata: ComponentContextMetaData;
}
...
class ApplicationContextInstance /*implements ApplicationContextProviderProps*/ {
    ...
    constructor() {
        this.applicationContext = {
            componentDidRenderServerSideFuncs: [],
            tasks: [],
            firstRun: true,
            cssUrls: [],
            jsUrls: [],
...
function getHypernovaPort(): string | number {
    return process.env.PORT || defaultHypernovaPort;
}

function getTimeOutMilliseconds(metadata: ComponentContextMetaData): number {
...
function getBaseUrl(metadata: ComponentContextMetaData): string {
...
function wrapWithTimeout(promise: Promise<any>, timeoutMilliseconds: number, timeoutRejectionValue: string): Promise<any> {
...
function getComponentAsyncRedux(component: any, context: ComponentContext) {
    ...
    applicationContextServer.applicationContext.isAmp = metadataApplicationContextServer.isAmp || false;
    ...
    let taskCompletionPromiseResolve: (value?: {} | PromiseLike<{}>) => void;
    let taskCompletionPromiseReject: (value?: {} | PromiseLike<{}>) => void;
...
function createCustomGetComponent(
    bundlesConfiguration: { [key: string]: string },
    vmOptions: CreateVmOptions,
    bundleLoaderPromises: Promise<any>[]
) {
    let resolvedBundles: { [key: string]: any } = {};
        ...
        for (let bundleName in bundlesConfiguration) {
            ...
            if (bundlePath.startsWith("http")) {
                ...
                const promise = new Promise((resolve, reject) => {
                    ...
                    const request = client.get(bundlePath, (res: http.IncomingMessage) => {
                        if (res.statusCode === 200 &&
                            res.headers && res.headers['content-type'] && res.headers['content-type'].includes('javascript')) {
                            let code = '';
                            res.setEncoding('utf8');
                            res.on('data', (chunk: string) => { code += chunk; });
                            res.on('end', () => {
                                try {
    ...
    return (componentSpecifier: string, context: ComponentContext) => {
...
let bundleLoaderPromises: any[] | null = []; // all promises that must be resolved so we know all required server bundles are loaded
let bundlesConfiguration;

if (typeof bundleToDebug === 'undefined') {
...
let app = hypernova({
    ...
    getComponent: createCustomGetComponent(bundlesConfiguration, {} as CreateVmOptions, bundleLoaderPromises),
...
Promise.all(bundleLoaderPromises).then(() => {
    ...
    app.get('/mem', (req: Request, res: Response) => {
  ...
  "rss (Resident Set Size - total space allocates in memory)": "${formatNumber(rss).format('0.00 ib')}", 
  "heapTotal (Total Size of the Heap)": "${formatNumber(heapTotal).format('0.00 ib')}",
  "heapUsed (Heap actually Used)": "${formatNumber(heapUsed).format('0.00 ib')}",
